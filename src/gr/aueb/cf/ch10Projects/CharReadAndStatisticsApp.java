...
/**
 * Αναπτύξτε μια εφαρμογή που διαβάζει έναν-έναν τους χαρακτήρες
 * ενός αρχείου και τους εισάγει σε ένα πίνακα 256χ2. Κάθε θέση
 * του πίνακα είναι επομένως ένας πίνακας δύο θέσεων, όπου στη 1η
 * θέση αποθηκεύεται ο χαρακτήρας που έχει διαβαστεί (αν δεν υπάρχει
 * ήδη στον πίνακα) και στην 2η θέση αποθηκεύεται το πλήθος των φορών
 * που έχει διαβαστεί (βρεθεί) κάθε χαρακτήρας. Χαρακτήρες θεωρούνται
 * και τα κενά οι αλλαγές γραμμής και γενικά οποιοσδήποτε UTF-8 χαρακτήρας.
 * Στο τέλος η main παρουσιάζει στατιστικά στοιχεία γαι κάθε χαρακτήρα
 * και ανά συχνότητα εμφάνισης.
 */
public class CharReadAndStatisticsApp {
    final static Path path = Paths.get("C:/temp/logTextStatistics.txt");
    ...
    public static void readTextAndSaveService() throws IOException, IllegalArgumentException {
        ...
        try (FileInputStream fs = new FileInputStream("C:/temp/inputText.txt");) {
    ...
    /*
    For each char in the input text, prints the count and the percentage,
    sorted by char asc and percentage asc.
     */
    ...
    /**
     * Inserts a char in a 2D array together with its occurrences'
     * count in the text.
     * @param ch    the char
